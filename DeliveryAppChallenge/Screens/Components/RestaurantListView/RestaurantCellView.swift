...
class RestaurantCellView: UITableViewCell {
    ...
    static let cellIdentifier = "RestaurantCellIdentifier"
...
extension RestaurantCellView {
    func setupCell(_ restaurant: Restaurant) {
        ...
        restaurantInfoLabel.text = formatInfo(restaurant)
    }
    
    private func formatInfo(_ restaurant: Restaurant) -> String {
        return "\(restaurant.category) • \(restaurant.deliveryTime.min)-\(restaurant.deliveryTime.max) min"
    }
}
